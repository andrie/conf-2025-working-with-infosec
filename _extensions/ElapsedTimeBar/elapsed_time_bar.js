...
const initTimer = function(deck) {
  const config = deck.getConfig();
  const options = config.ElapsedTimeBar || {}; 
  console.log('options', options)
  console.log('allottedTime', options.allottedTime);
  window.addEventListener('ready', () => _ElapsedTimeBar.handleReady(options));
}


window.ElapsedTimeBar = window.ElapsedTimeBar || function() {
  return {
    id: 'ElapsedTimeBar',
    init: (deck) => {
      initTimer(deck);
